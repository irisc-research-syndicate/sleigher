...
impl<'sleigh> Disassembler<'sleigh> {
    ...
    pub fn disassemble(&'sleigh self, inst_start: u64, context: Context, bytes: &[u8]) -> Result<DisassembledInstruction<'sleigh>> {
        let table = self.disassemble_table(inst_start, self.table(self.instruction_table()), Context, bytes)?;
        Ok(DisassembledInstruction {
           table
        })
    }

    pub fn disassemble_table(&'sleigh self, inst_start: u64, table: &'sleigh Table, context: Context, bytes: &[u8]) -> Result<DisassembledTable<'sleigh>> {
    ...
    pub fn extract_token_field(&self, token_field_id: TokenFieldId, bytes: &[u8]) -> i64 {
        ...
        log::trace!("Extracting {} from {}/{:?}: {:02x?}", token_field.name(), token.name(), token.endian, token_bytes);

        let token_value = match token.endian {
            Endian::Little => token_bytes.iter().rev().fold(0u64, |n, b| (n << 8) | (*b as u64)),
            Endian::Big => token_bytes.iter().fold(0u64, |n, b| (n << 8) | (*b as u64))
        };

        log::trace!("Token value: {:#x}", token_value);

        let token_field_raw = token_value >> token_field.bits.start() & ((1 << token_field.bits.len().get()) - 1);

        log::trace!("Token field raw: {:#x}", token_field_raw);

        let token_field_value = if token_field.raw_value_is_signed() && (token_field_raw & (1 << (token_field.bits.len().get() - 1)) != 0) {
...
pub fn bitconstraint_to_string(constraints: &[BitConstraint]) -> String {
    constraints.iter().map(|constraint| match constraint {
...
impl<'sleigh> DisassembledTable<'sleigh> {
    ...
    pub fn disassemble(disassembler: &'sleigh Disassembler<'sleigh>, inst_start: u64, table: &'sleigh Table, mut bytes: &[u8]) -> Result<Self> {
        ...
        'match_loop: for matcher in table.matcher_order() {
            ...
            if log::log_enabled!(log::Level::Trace) {
                log::trace!("Constext constraint: {}", bitconstraint_to_string(context_constraints));
                log::trace!("Data constraint: {}", bitconstraint_to_string(data_constraints));
            }
            ...
            for block in constructor.pattern.blocks() {
                ...
                for produced_token_field in block.token_fields() {
                    let token_field_value = disassembler.extract_token_field(produced_token_field.field, bytes);
                    disasm_table.token_fields.insert(produced_token_field.field, token_field_value);
                }

                for produced_table in block.tables() {
                    let subtable = disassembler.table(produced_table.table);
                    if let Ok(subtable_value) = DisassembledTable::disassemble(disassembler, inst_start, subtable, bytes) {
                        disasm_table.tables.insert(produced_table.table, subtable_value);
                    } else {
                        log::trace!("{}: failed to disassemble subtable {}, continuing to next matcher", table.name(), subtable.name());
                        continue 'match_loop;
                    }
                }
                ...
                for verification in block.verifications() {
                    match verification {
                        ...
                        Verification::TableBuild { produced_table, verification } => {
            ...
            Expr::Value(expr_element) => {
                match expr_element {
                    ExprElement::Value { value, location } => match *value {
                        ...
                        ReadScope::TokenField(token_field_id) => self.disassembler.extract_token_field(token_field_id, bytes),
                        ReadScope::InstStart(_inst_start) => self.inst_start as i64,
                        ReadScope::InstNext(_inst_next) => self.inst_next as i64,
                        ReadScope::Local(variable_id) => self.variables.get(&variable_id).unwrap().clone(),
                    },
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        for display_element in self.constructor.display.elements() {
            parts.push(match display_element {
                DisplayElement::Varnode(varnode_id) => self.disassembler.varnode(*varnode_id).name().to_string(),
                DisplayElement::Context(context_id) => self.disassembler.context(*context_id).name().to_string(),
                    ...
                    if let Some(token_field_value) = self.token_fields.get(&token_field_id).cloned() {
                        match token_field.attach {
                            ...
                            TokenFieldAttach::Varnode(attach_varnode_id) => {
                                let attach_varnode = self.disassembler.attach_varnode(attach_varnode_id);
                                if let Some(varnode_id) = attach_varnode.find_value(token_field_value as usize) {
                            ...
                            TokenFieldAttach::Literal(attach_literal_id) => {
                                let attach_literal = self.disassembler.attach_literal(attach_literal_id);
                                if let Some(literal_str) = attach_literal.find_value(token_field_value as usize) {
                            ...
                            TokenFieldAttach::Number(print_base, attach_number_id) => {
                                let attach_number = self.disassembler.attach_number(attach_number_id);
                                if let Some(number) = attach_number.find_value(token_field_value as usize) {
                                    match (print_base, number) {
                                        (sleigh_rs::PrintBase::Dec, sleigh_rs::Number::Positive(x)) => format!("{}", x),
                                        (sleigh_rs::PrintBase::Dec, sleigh_rs::Number::Negative(x)) => format!("-{}", x),
                                        (sleigh_rs::PrintBase::Hex, sleigh_rs::Number::Positive(x)) => format!("{:#x}", x),
                                        (sleigh_rs::PrintBase::Hex, sleigh_rs::Number::Negative(x)) => format!("-{:#x}", x),
                                    }
                                } else {
                        ...
                        format!("<UNDEFINED FIELD {}/{}>", token_field.name(), token_field_id.0)
                    }
                },
                DisplayElement::InstStart(_inst_start) => format!("{:x}", self.inst_start),
                DisplayElement::InstNext(_inst_next) => format!("{:x}", self.inst_next),
                DisplayElement::Table(table_id) => format!("{}", self.tables.get(table_id).unwrap()),
